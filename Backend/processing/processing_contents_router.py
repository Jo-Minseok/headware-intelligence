...
def processing_data(db: Session = Depends(get_db), manager: str = Path(...), situationCode: str = Path(...)):
    # 사고 처리 데이터 조회
    accidentProcessings = processing_contents_crud.get_accident_processings(
        db=db, manager=manager, situationCode=situationCode)
    ...
    try:
        for accidentProcessing in accidentProcessings:
            accident = processing_contents_crud.get_accident(
                db=db, no=accidentProcessing.no)
            ...
            victim.append(processing_contents_crud.get_victim_name(
                db=db, no=accidentProcessing.no))
